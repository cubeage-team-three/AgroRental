...
function PartnerDashboard() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        const profileData = await partnerService.getProfile(partnerId);
        if (profileData) {
        ...
        if (!partner) {
          setPartner({
            ...
            address: 'Krishi Seva Kendra, Pune',
          });
        }
      } finally {
        setLoading(false);
  ...
  const partnerName = partner?.fullName || currentUser?.fullName || 'Partner';
  const businessName = partner?.businessName || currentUser?.businessName || 'Agro Machinery Fleet';
  const verificationStatus = partner?.verificationStatus || 'APPROVED';
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          ...
          <span className="text-[11px] text-emerald-600 font-semibold mt-1 block">
            {availableCount} Active & Ready for Rent
          </span>
        </Link>
